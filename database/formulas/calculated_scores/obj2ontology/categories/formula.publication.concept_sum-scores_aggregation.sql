...
REPLACE INTO [[graph_cache]].Edges_N_Object_N_Category_T_CalculatedScores
  ...
  INNER JOIN [[airflow]].Operations_N_Object_T_ScoresExpired se
         ...
         AND tf.object_type = 'Publication'
         AND tf.flag_type   = 'scores'
         AND tf.to_process  = 1
         AND se.object_type = 'Publication'
         AND se.to_process  = 1

          -- Join traversal
  INNER JOIN [[graph_cache]].Traversal_N_Publication_N_Concept_T_ConceptDetection t
          ON se.institution_id = t.institution_id
         AND se.object_id = t.publication_id
