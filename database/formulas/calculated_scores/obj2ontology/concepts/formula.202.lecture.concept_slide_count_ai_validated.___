...
REPLACE INTO [[graph_cache]].Edges_N_Object_N_Concept_T_CalculatedScores
      ...
      SELECT 'EPFL' AS institution_id, 'Lecture' AS object_type, lecture_id AS object_id, concept_id,
        ...
        FROM (SELECT p.to_object_id AS lecture_id, v.object_id AS slide_id, concept_id, c.field_value AS n_slides
                FROM [[lectures]].Edges_N_Object_N_Concept_T_LLMPostValidated v
          INNER JOIN [[lectures]].Edges_N_Object_N_Object_T_ChildToParent p
                  ON p.from_object_id = v.object_id
          INNER JOIN [[lectures]].Data_N_Object_T_CustomFields c
