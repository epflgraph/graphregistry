...
class TestNodeKey:
    def test_node_key_from_dict(self) -> None:
        ...
        assert key.to_tuple() == ("Course", "CS-433")

    def test_node_key_from_tuple(self) -> None:
        key = NodeKey.from_tuple(("Person", "p-1"))
...
class TestNodeKeyList:
    def test_from_tuple_list(self) -> None:
        key_list = NodeKeyList.from_tuple_list([("Course", "CS-433"), ("Course", "MATH-203")])
        assert len(key_list.item_list) == 2
        assert key_list.to_tuple_list() == [("Course", "CS-433"), ("Course", "MATH-203")]


class TestNodeFieldKey:
    def test_from_tuple(self) -> None:
        key = NodeFieldKey.from_tuple(("Course", "CS-433", "en", "level"))
        assert key.key.to_tuple() == ("Course", "CS-433")
...
class TestEdgeKey:
    def test_edge_key_from_dict(self) -> None:
        ...
        assert key.to_tuple() == ("Course", "CS-433", "Person", "p-1", "taught_by")

    def test_edge_key_from_tuple(self) -> None:
        key = EdgeKey.from_tuple(("Course", "CS-433", "Person", "p-1", "taught_by"))
        assert key.context == "taught_by"

    def test_edge_key_from_tuple_wrong_length(self) -> None:
        with pytest.raises(ValidationError):
            EdgeKey.model_validate(("Course", "CS-433"))


class TestEdgeKeyList:
    def test_from_tuple_list(self) -> None:
        key_list = EdgeKeyList.from_tuple_list([
            ("Course", "CS-433", "Person", "p-1", "taught_by"),
            ("Course", "MATH-203", "Person", "p-2", "taught_by"),
        ])
        assert len(key_list.item_list) == 2
...
class TestEdgeFieldKey:
    def test_from_tuple(self) -> None:
        key = EdgeFieldKey.from_tuple(("Course", "CS-433", "Person", "p-1", "taught_by", "en", "semester"))
