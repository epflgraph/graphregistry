...
(from_object_type, from_object_id, to_object_type, to_object_id, context, field_language, field_name, field_value)

SELECT 'Person' AS from_object_type,
       from_id  AS from_object_id,
       'Unit'   AS to_object_type,
       to_id    AS to_object_id,
       'accreditation' AS context,
       'n/a'    AS field_language,
       'longest_position_name' AS field_name,
       COALESCE(position_name_en, position_name_fr_male) AS field_value
FROM (
    SELECT e.*,
           ROW_NUMBER() OVER (
               PARTITION BY from_id, to_id
               ORDER BY DATEDIFF(COALESCE(end_date, NOW()), start_date) DESC
           ) AS rn
