...
def cmd_cache_to_process(args):
    ...
    list_of_tables = [
        ...
        if 'to_process' in db.get_column_names(    engine_name='coresrv', schema_name=glbcfg.schema_graph_cache_test, table_name=table_name)
    ]
    ...
    if args.count:
        ...
        for table_name in list_of_tables:
            n = db.count_rows_in_table(engine_name='coresrv', schema_name=glbcfg.schema_graph_cache_test, table_name=table_name, where_clause="to_process = 1")
    ...
    # >> graphregistry cache to_process --reset
        ...
        for table_name in list_of_tables:
            ...
            db.set_cells(engine_name='coresrv', schema_name=glbcfg.schema_graph_cache_test, table_name=table_name, set=[('to_process', 0)], where=[('to_process', 1)], verbose=False)
...
def cmd_cache_debug(args):
    ...
    # db.print_database_stats(engine_name='coresrv', schema_name='graphsearch_test'   , re_exclude=[r'.*(MOOC|Lecture|Widget).*'])
    # db.print_database_stats(engine_name='coresrv', schema_name='elasticsearch_cache', re_exclude=[r'.*(MOOC|Lecture|Widget).*'])
    ...
    # glbcfg = GlobalConfig()
    # db.print_database_stats(engine_name='coresrv', schema_name=glbcfg.schema_names['graphsearch_test']   , re_exclude=[r'.*(MOOC|Lecture|Widget|Notebook|Exercise|Specialisation|Startup|StudyPlan).*'])
    # db.print_database_stats(engine_name='coresrv', schema_name=glbcfg.schema_names['elasticsearch_cache'], re_exclude=[r'.*(MOOC|Lecture|Widget|Notebook|Exercise|Specialisation|Startup|StudyPlan).*'])
